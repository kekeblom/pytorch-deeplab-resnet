...
import timeit
from convert import convert_from_color_segmentation
...
Options:
    ...
    -b, --batchSize=<int>       Training batch size
"""
...
def scale_im(img_temp, scale):
    new_dims = (int(img_temp.shape[0] * scale),
                int(img_temp.shape[1] * scale))
...
def preprocess_images(dataset_images, dataset_labels):
    ...
    images = np.zeros((dim, dim, 3, image_count))
    gt = np.zeros((dim, dim, 1, image_count))
    for i, img_temp in enumerate(dataset_images):
...
optimizer = optim.SGD([{'params': get_1x_lr_params_NOscale(model), 'lr': base_lr}, {
                      'params': get_10x_lr_params(model), 'lr': 10 * base_lr}], lr=base_lr, momentum=0.9, weight_decay=weight_decay)

optimizer.zero_grad()

dataset = ImageDataset(img_list, args['--IMpath'], args['--GTpath'])
data_loader = DataLoader(dataset,
        batch_size=batch_size,
        sampler=RandomSampler(dataset),
        num_workers=2)

for iter, sampled_batch in enumerate(data_loader):
    images, label = sampled_batch
    images, label = preprocess_images(images.numpy(), label.numpy())
    ...
    if iter % iter_size == 0:
        ...
        optimizer = optim.SGD([{'params': get_1x_lr_params_NOscale(model), 'lr': lr_}, {
                              'params': get_10x_lr_params(model), 'lr': 10 * lr_}], lr=lr_, momentum=0.9, weight_decay=weight_decay)
        optimizer.zero_grad()
